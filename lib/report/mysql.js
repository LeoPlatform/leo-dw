...
var pool = {
	query: function (query, callback, config) {
...
function queryFactTable(factTable, calcLookups, options, repeatableSha1s, callback) {
	...
	if (options.query) {
		...
		select += " From " + mysql.escapeId(factTable.table);
		for (var alias in requiredTables) {
			...
			if (filtered) {
				...
				if (dim.on) {
					...
					joins.push("JOIN `" + dim.table + "` as " + dim.alias + " on " + dim.alias + "._id = " + mysql.escapeId(dim.joinTable) + "." + mysql.escapeId(dim.alias.split('$')[1]));
				} else {
					joins.push("JOIN `" + dim.table + "` as " + dim.alias + " on " + dim.alias + "._id = " + mysql.escapeId(factTable.table) + "." + mysql.escapeId(dim.alias));
				}
			} else {
				if (dim.on) {
					if (typeof dim.on == "string") {
					...
					joins.push("STRAIGHT_JOIN `" + dim.table + "` as " + dim.alias + " on " + dim.alias + "._id = " + mysql.escapeId(dim.joinTable) + "." + mysql.escapeId(dim.alias.split('$')[1]));
				} else {
					joins.push("STRAIGHT_JOIN `" + dim.table + "` as " + dim.alias + " on " + dim.alias + "._id = " + mysql.escapeId(factTable.table) + "." + mysql.escapeId(dim.alias));
				}
			}
		}
