...
import {parse_date} from '../../../lib/utils/parse_date.js';
...
export default class DateRangePicker extends React.Component {
    ...
    constructor(props) {
        ...
        if (!this.props.filter.value) {
            ...
            } else if (state.defaultValue in presets) {
                state.custom = false;
                state.last = presets[state.defaultValue][0];
                state.period = presets[state.defaultValue][1];
                state.to_date = presets[state.defaultValue][2];
                state.current = presets[state.defaultValue][3];

            } else {
                Object.keys(presets).map((preset) => {
                    let dates = parse_date(preset).map((date) => {
        ...
        if (state.custom == true) {
            if (this.props.filter.value[0] && this.props.filter.value[0].match(/\d{4}\-\d{2}\-\d{2}/)) {
                ...
                let defaults = state.defaultValue.split(/[ +]+/g);
    ...
    initCalendars() {
        ...
        if (!$('#custom-date-range > div').hasClass('hasDatepicker')) {
            ...
            $('#custom-date-range').dateRangePicker({
                ...
                firstDay: 0,
                onChange() {
                    let range = $('#custom-date-range').dateRangePicker('getRange');
                    if (range) {
            ...
            if (!this.state.state.range) {
                if (this.state.state.atDateRange) {
                    let dates = parse_date(this.state.defaultValue);
                    $('#custom-date-range').dateRangePicker("setRange", {start: new Date(dates[0]), end: new Date(dates[1] + ' 23:59:59') })
                } else {
                    let date = 'Last ' + this.refs.customLast.value + ' ' + this.refs.customPeriod.value + (this.refs.customCurrent.checked ? '+current' : '')+(this.refs.customToDate.value == 'period' ? '' : this.refs.customToDate.value);
                    let dates = parse_date(date);
                    $('#custom-date-range').dateRangePicker("setRange", {start: new Date(dates[0]), end: new Date(dates[1] + ' 23:59:59') })
                }
            } else {
                $('#custom-date-range').dateRangePicker("setRange", {start: new Date(thisComponent.state.state.start), end: new Date(thisComponent.state.state.end + ' 23:59:59') })
            }
        ...
        if ($(this.refs.presetList).closest('.filter-select').hasClass('flow-up')) {
            let off = $(this.refs.presetList).closest('.filter-wrapper').offset()
            $(this.refs.presetList).css({ maxHeight: (off.top-400) })
        } else {
            let off = $(this.refs.presetList).offset()
    ...
    setCustomRange(range) {
        ...
        this.state.clicked++;
        if (this.state.loading && this.state.clicked === 2) {
    ...
    setAtDateRange() {
        let date = '@daterange(' + this.refs.dateRangeStart.value + (this.refs.dateRangeStart.value && this.refs.dateRangeEnd.value ? '-' : '') + this.refs.dateRangeEnd.value + ' ' + this.refs.dateRangePeriod.value + ' ago)'
        this.props.setRange([date]);
        let dates = parse_date(date);
        this.state.loading = false;
        $('#custom-date-range').dateRangePicker("setRange", {start: new Date(dates[0]), end: new Date(dates[1] + ' 23:59:59') });
    ...
    render() {
        ...
        return (
            <div>
            {
                ...
                !this.state.state.atDateRange

                    ? <div className="range-custom">
                        <fieldset className={!this.state.state.range ? 'active' : ''}>
                            <legend>Custom Timespan</legend>
                            <div>
                                Last <input ref="customLast" className="new-styled" type="number" min="0" defaultValue={this.state.state.last} onChange={this.changedCustom.bind(this, 'last')} />
                                <select ref="customPeriod" className="new-styled" defaultValue={this.state.state.period} onChange={this.changedCustom.bind(this, 'period')}>
                                    {Object.keys(this.state.periods).map((period, i) => {
                                        return <option key={i} value={period}>{this.state.periods[period]}</option>
                                    })}
                                </select>
                            </div>
                            <div>
                                <select ref="customToDate" className="new-styled" defaultValue={this.state.state.to_date} onChange={this.changedCustom.bind(this, 'to_date')}>
                                    {Object.keys(this.state.toDates).map((toDate, i) => {
                                        return <option key={i} value={toDate}>{this.state.toDates[toDate]}</option>
                                    })}
                                </select>
                            </div>
                            <div>
                                <label>
                                    <input ref="customCurrent" type="checkbox" defaultChecked={this.state.state.current} onChange={this.changedCustom.bind(this, 'current')} /> Include current <span></span>
                                </label>
                            </div>
                        </fieldset>
                    ...
                    : <div className="range-custom">
                        <fieldset className={!this.state.state.range ? 'active' : ''}>
                            ...
                            <div>
                                <input ref="dateRangeStart" className="new-styled" type="number" min="0" defaultValue={this.state.state.atDateRange[0]} onChange={this.changedAtDateRange.bind(this, 'start')} />
                                <span> &mdash; </span>
                                <input ref="dateRangeEnd" className="new-styled" type="number" min="0" defaultValue={this.state.state.atDateRange[1]} onChange={this.changedAtDateRange.bind(this, 'end')} />
                                <select ref="dateRangePeriod" className="new-styled" defaultValue={this.state.state.atDateRange[2]} onChange={this.changedAtDateRange.bind(this, 'period')}>
                                    {Object.keys(this.state.periods).map((period, i) => {
                ...
                !this.state.state.atDateRange
                    ...
                    ? <div className="range-presets">
                        <div className="preset-list" ref="presetList">
                            <ul>
                                {
                                    Object.keys(this.state.presets).map((preset, index) => {
                                        let parsed = parse_date(preset);
                                        let dates = parsed.map((date) => {
                                            return this.formatDate(date)
                                        }).join(' - ');
                                        return (<li key={index}>
                                            <button type="button" className={preset.toLowerCase() == this.state.defaultValue ? 'active' :''} onClick={this.setRange.bind(this, preset.capitalize())}>{preset.capitalize()}</button>
                                            <button type="button" className={dates.toLowerCase() == this.state.defaultValue ? 'active' : ''} onClick={this.setRange.bind(this, parsed)}>{dates}</button>
                                        </li>)
                                    })
                                }
                            </ul>
