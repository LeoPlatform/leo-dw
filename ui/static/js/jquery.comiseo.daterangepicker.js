...
(function($, window, undefined) {
	...
	$.widget('comiseo.daterangepicker', {
		...
		options: {
			...
			onApply: null, // callback that executes when the submit button is clicked
			...
			useMonthTrigger: true, //allow clicking on month name to select whole month
			useWeekTrigger: true, //allow clicking on week number to select whole weel
...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildDateRangePicker($originalElement, options) {
		...
		function usePreset() {
			...
			calendar.setRange({ start: start, end: end }, true);
			if (options.applyOnMenuSelect || options.inline) {
				close();
				setRange();
				if (options.applyOnMenuSelect && options.onApply) {
					options.onApply();
				}
			}
			return false;
		...
		function open() {
			if (!isOpen) {
				...
				if (options.useMonthTrigger) {
					options.useMonthTrigger = false;
					$container.on('click', '.ui-datepicker-title', function() {
						//select whole month
						var year = $(this).find('.ui-datepicker-year').text();
						var month = $(this).find('.ui-datepicker-month').text();
						var periodStart = moment().year(year).month(month).startOf('month').toDate();
						var periodEnd = moment().year(year).month(month).endOf('month').toDate();
						calendar.setRange({start: periodStart, end: periodEnd }, false);
						if (options.applyOnMenuSelect || options.inline) {
							close();
							setRange();
							if (options.applyOnMenuSelect && options.onApply) {
				...
				if (options.useWeekTrigger) {
					options.useWeekTrigger = false;
					$container.on('click', '.ui-datepicker-week-col', function() {
						var year = $(this).closest('.ui-datepicker-group').find('.ui-datepicker-year').text();
						if (!year) year = $(this).closest('.ui-datepicker').find('.ui-datepicker-year').text();
						if (year) {
							var periodStart = moment().year(year).week($(this).text()).startOf('isoWeek').toDate();
							var periodEnd = moment().year(year).week($(this).text()).endOf('isoWeek').toDate();
							calendar.setRange({start: periodStart, end: periodEnd }, false);
							if (options.applyOnMenuSelect || options.inline) {
								close();
								setRange();
