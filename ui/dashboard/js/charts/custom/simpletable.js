...
var format = require("../../../../../lib/report/format/format.js");
...
module.exports = function (element, spec, options, my) {
	...
	my.redraw = function () {
		...
		return <SimpleTable spec={spec} options={options} data={my.dataSources[0]} element={element} />
	}
...
var SimpleTable = React.createClass({
	...
	render: function () {
		...
		return (<div className="simple-table-wrapper">
					...
					<thead className="table-head">
						...
						<tr className="title">
							<td colSpan={this.columns.length}>
								<i title="download" className="icon-download" onClick={this.exportData.bind(this, this.rows, this.columns)}></i>
								<span>{this.props.options.title || this.props.spec.title}</span>
							</td>
						</tr>
						<tr className="headers">
							{this.columns.map((column,i) => {
								return (<td key={i} style={{width: column.width, maxWidth: column.width}} className={column.className + (this.state.sortBy == i ? ' active' : '')} onClick={this.sortBy.bind(this, i)}>
						...
						<tr className="filters">
							{this.columns.map((column,i) => {
								if (column.type != "metric" && !column.className.match('numeric') && column.filter !== false) {
									return (<td key={i} style={{width: column.width, maxWidth: column.width}} className={column.className}>
			...
			<section className={"table-body" + (this.props.spec.showTotals ? ' has-totals' : '')} onScroll={this.handleScroll}>
				<table className={className}>
					<tbody>
						<tr className="fixed-spacing">
							{this.columns.map((column,i) => {
						...
						{this.outRows.slice(this.state.startRow, this.state.startRow + this.visibleRowCount).map((row, rowNum) => {
							return (<tr key={rowNum}>
								{this.columns.map((column, i) => {
									return <td key={i} style={{width: column.width, maxWidth: column.width}} className={column.className} dangerouslySetInnerHTML={{__html: column.formatter?column.formatter(row[i]):row[i] }} />
								})}
							</tr>)
						})}
					...
					</tbody>
					{
						...
						? <tfoot>
							<tr>
								{this.columns.map((column,i) => {
									return <td key={i} style={{width: column.width, maxWidth: column.width}} className={column.className} dangerouslySetInnerHTML={{__html: (column.formatter ? column.formatter(totals[i]) : totals[i]) || '&nbsp;' }} />
								})}
							</tr>
						</tfoot>
						: false
